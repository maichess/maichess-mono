...
class PgnSuite extends FunSuite:
  ...
  test("SAN encode: check suffix"):
    ...
    val sit = Situation(Board(pieces), Color.White, CastlingRights.none, None, 0, 1)
    val m   = move("d1", "d8")
    val san = San.encode(sit, m, after(sit, m), StandardRules)
    assert(san.endsWith("+"), s"Expected check suffix, got '$san'")
  ...
  test("SAN encode: checkmate suffix"):
    ...
    val san    = San.encode(before, moves(3), aft, StandardRules)
    assert(san.endsWith("#"), s"Expected checkmate suffix, got '$san'")
